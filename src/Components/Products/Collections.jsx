...
const Collections = () => {
  ...
  // products count
  const productCountsByCategory = allProducts.reduce((acc, product) => {
            ...
            <div className="flex flex-col gap-2">
              {categories.map(({ catValue, catName }) => (
                <button
                  ...
                  className={`text-left px-2 py-1.5 rounded-md border ${selectedCategory === catValue
                    ? "cat_tab_bg_select"
                    : "bg-white text-gray-700"
                    } cat_tab_hover flex justify-between items-center`}
                >
                  <span>
                    {catName}
                    <span className="ml-2 text-sm text-yellow-600">
                      ...
                      {catValue === "all"
                        ? allProducts.length
                        : productCountsByCategory[catValue] || 0}
                      )
                    </span>
                  </span>
                  <FaAngleRight />
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {currentSubCategories.length === 0 ? (
                  ...
                  currentSubCategories.map(({ subCatValue, subCatName }) => (
                    <button
                      key={subCatValue}
                      onClick={() => {
                      ...
                      className={`text-left px-4 py-1.5 rounded-md border ${selectedSubCategory === subCatValue
                        ? "cat_tab_bg_select"
                        : "bg-white text-gray-700"
                        } cat_tab_hover`}
                    >
            ...
            <div className="flex flex-col gap-3">
              {priceRanges.map(({ label, range }) => (
                <button
                  ...
                  className={`text-left px-4 py-1.5 rounded-md border ${selectedPriceRange === range
                    ? "cat_tab_bg_select"
                    : "bg-white text-gray-700"
                    } cat_tab_hover`}
                >
            ...
            <div className="flex flex-col gap-2">
              {colors.map(({ label, value }) => (
                <button
                  ...
                  className={`text-left px-4 py-1.5 rounded-md border ${selectedColor === value
                    ? "cat_tab_bg_select"
                    : "bg-white text-gray-700"
                    } cat_tab_hover`}
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 ">
              {currentProducts.map((product) => (
                <Link key={product._id} to={`/product/${product._id}`}>
                  <div className="border relative rounded-lg p-2 shadow-sm hover:shadow-md transition group cursor-pointer overflow-hidden">
                    {product.popular && (
