...
const Products = () => {
  ...
  // popular products filter (ধরি popular: true আছে product এ)
          ...
          {popularProducts.length > 0 ? (
            <ul className="flex flex-col gap-4">
              {popularProducts.map((product) => (
                <li key={product._id} className="border rounded p-2 hover:shadow cursor-pointer transition">
                  <Link to={`/product/${product._id}`} className="flex items-center gap-3">
                    <img
                      src={product.image}
                      alt={product.title}
